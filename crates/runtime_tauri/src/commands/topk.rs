...
pub async fn create_ranking_view(
    ...
    match application::topk::execute_ranking(&conn, &token, &dto.ranking_view_id).await {
        Ok(result) => {
            let _ = application::topk::create_snapshot(&conn, &dto.ranking_view_id, &result.items);
        }
        Err(e) => {
            tracing::warn!(
...
pub async fn execute_ranking(app: AppHandle, view_id: String) -> Result<RankingResultDto, String> {
    let conn = get_db_connection(&app)?;
    let token = load_token()?;
    let mut result = application::topk::execute_ranking(&conn, &token, &view_id)
        .await
        .map_err(|e| e.to_string())?;

    // 执行后自动保存快照
    let _ = application::topk::create_snapshot(&conn, &view_id, &result.items);

    // 计算排名变化
    let rank_changes = application::topk::get_rank_change(&conn, &view_id, &result.items)
        .map_err(|e| e.to_string())?;
